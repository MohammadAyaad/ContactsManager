package com.MohammadAyaad.ContactsManager.Data;
...
import com.MohammadAyaad.ContactsManager.Model.Contact.*;
import com.MohammadAyaad.ContactsManager.Model.Email.Email;
import com.MohammadAyaad.ContactsManager.Model.Email.InvalidEmailAddressException;
import com.MohammadAyaad.ContactsManager.Model.PhoneNumber.InvalidPhoneNumberException;
import com.MohammadAyaad.ContactsManager.Model.PhoneNumber.PhoneNumber;
...
public class ContactsManager {
    ...
    public List<ContactBasic> searchContact(String term) throws SQLException {
        String rterm = term;
        String qry = "SELECT Contacts.*,(MIN((CASE WHEN (Contacts.firstName IS NOT NULL AND Contacts.lastName IS NOT NULL) THEN EDITD((Contacts.firstName || ' ' || Contacts.lastName),'"+rterm+"') ELSE 2147483647 END),(CASE WHEN PhoneNumbers.phoneNumber IS NOT NULL THEN EDITD(CAST(PhoneNumbers.phoneNumber AS TEXT),'"+rterm+"') ELSE 2147483647 END),(CASE WHEN emails.email IS NOT NULL THEN EDITD(emails.email,'"+rterm+"') ELSE 2147483647 END)) / MIN(LENGTH((Contacts.firstName || ' ' || Contacts.lastName)),LENGTH(CAST(PhoneNumbers.phoneNumber AS TEXT)),LENGTH(emails.email))) AS distance FROM Contacts LEFT JOIN PhoneNumbers ON PhoneNumbers.contact_id = Contacts.id LEFT JOIN emails ON emails.contact_id = Contacts.id GROUP BY Contacts.id ORDER BY distance ASC,(Contacts.firstName || ' ' || Contacts.lastName) DESC;";
        System.out.println(qry);
